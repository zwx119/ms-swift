...
class Seq1F1BFlashAttnVarlenFunc(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        # Backward visits chunks in reverse order. Pop the current chunk so the
        # previous chunk sees the same prefix it had during its forward pass.
        ctx.kv_cache['key'] = full_key[:, :-ctx.seqlen_q].contiguous()
        ctx.kv_cache['value'] = full_value[:, :-ctx.seqlen_q].contiguous()
        ...
        if cached_dk is not None:
            ...
            if cached_len > 0:
                dk[:, :cached_len] = dk[:, :cached_len] + cached_dk
                dv[:, :cached_len] = dv[:, :cached_len] + cached_dv
...
class Seq1F1BHybridSelfAttention(SelfAttention):
    ...
    def __init__(
        ...
        self.seq1f1b_recompute_kv_init_cache = {}
    ...
    def _snapshot_kv_cache(self, cache):
        return {
            'key': self._clone_cache_tensor(cache.get('key', None)),
            'value': self._clone_cache_tensor(cache.get('value', None)),
        }

    @staticmethod
    def _restore_kv_snapshot(cache, snapshot) -> None:
    ...
    def forward(
        ...
        *,
        ...
        if micro_sp_idx == 0 and not (full_recompute_seq1f1b and torch.is_grad_enabled()):
            ...
            if not full_recompute_seq1f1b:
                self.seq1f1b_recompute_kv_init_cache = {}
        kv_cache = self.seq1f1b_kv_cache_by_microbatch.setdefault(cache_key, {})

        if full_recompute_seq1f1b:
            snapshot_key = (cache_key, int(micro_sp_idx))
            if not torch.is_grad_enabled():
                self.seq1f1b_recompute_kv_init_cache.setdefault(snapshot_key, []).append(
                    self._snapshot_kv_cache(kv_cache)
                )
            else:
                snapshot_stack = self.seq1f1b_recompute_kv_init_cache.get(snapshot_key)
                if snapshot_stack is None or len(snapshot_stack) == 0:
                    cached_key = kv_cache.get('key')
                    cached_value = kv_cache.get('value')
                    if cached_key is None:
                        if start != 0:
                            raise RuntimeError(
                                'Missing Seq1F1B hybrid softmax KV recompute snapshot for '
                                f'cache_key={cache_key}, micro_sp_idx={micro_sp_idx}, start={start}'
                            )
                    else:
                        if cached_value is None or cached_key.size(1) < start or cached_value.size(1) < start:
                            raise RuntimeError(
                                'Invalid Seq1F1B hybrid KV cache while rebuilding recompute prefix: '
                                f'cache_key={cache_key}, micro_sp_idx={micro_sp_idx}, start={start}, '
                                f'key_shape={getattr(cached_key, "shape", None)}, '
                                f'value_shape={getattr(cached_value, "shape", None)}'
                            )
                        kv_cache['key'] = cached_key[:, :start].contiguous()
                        kv_cache['value'] = cached_value[:, :start].contiguous()
                else:
                    self._restore_kv_snapshot(kv_cache, snapshot_stack.pop())
